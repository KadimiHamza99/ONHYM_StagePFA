...
public class DemandeServiceImpl implements DemandeService {
	...
	public DemandeAccesMessagerie addDemandeAM(FormInformationsAM formInfo, String username) {
		...
		emailNotificationService.notify(demandeur.getEmail(), OBJET_AM,
				MESSAGE_DEMANDEUR + demande.getIdDemandeAccesMessagerie());
		emailNotificationService.notify(demandeur.getManager().getEmail(), OBJET_AM,
				MESSAGE_VALIDATEUR + demandeur.getUsername());
	...
	public DemandeServiceSi addDemandeSI(FormInformationSI formInfo, String username) {
		...
		emailNotificationService.notify(demandeur.getEmail(), OBJET_SI,
				MESSAGE_DEMANDEUR + demande.getIdDemandeServiceSi());
		emailNotificationService.notify(demandeur.getManager().getEmail(), OBJET_SI,
				MESSAGE_VALIDATEUR + demandeur.getUsername());
	...
	public void managerValidationAM(String idDemande, String managerUsername) {
		...
		if (demande.getManager() == null && demande.getDemandeur().getManager() == validateur && !demande.isRefuser()) {
			...
			try {
				emailNotificationService.notify(demande.getDemandeur().getDsi().getEmail(), OBJET_AM,
						MESSAGE_VALIDATEUR + demande.getDemandeur().getUsername());
			} catch (Exception e) {
				demande.getDemandeur().getRoles().stream().forEach(role -> {
					if (role.getName().equals("DPI")) {
						...
						emailNotificationService.notifyWithAttachement(demande.getDemandeur().getEmail(), OBJET_AM,
	...
	public void dsiValidationAM(String idDemande, String dsiUsername) {
		...
		if (demande.getDsi() == null && demande.getDemandeur().getDsi() == validateur && !demande.isRefuser()) {
	...
	public void managerValidationSI(String idDemande, String managerUsername) {
		...
		if (demande.getManager() == null && demande.getDemandeur().getManager() == validateur && !demande.isRefuser()) {
			...
			try {
				emailNotificationService.notify(demande.getDemandeur().getDsi().getEmail(), OBJET_SI,
						MESSAGE_VALIDATEUR + demande.getDemandeur().getUsername());
			} catch (Exception e) {
				demande.getDemandeur().getRoles().stream().forEach(role -> {
					if (role.getName().equals("DPI")) {
						...
						emailNotificationService.notifyWithAttachement(demande.getDemandeur().getEmail(), OBJET_SI,
	...
	public void managerRefusAM(String idDemande, String managerUsername, String messageRefus) {
		...
		if (demande.getManager() == null && demande.getDemandeur().getManager() == refuseur
				&& demande.getEtatDemande() == (byte) -1) {
			dao.managerRefusAM(idDemande, managerUsername);
			emailNotificationService.notify(demande.getDemandeur().getEmail(), OBJET_AM,
					MESSAGE_REFUS + messageRefus + " REFUSER PAR => " + demande.getRefuseur());
		}
	}
	...
	public void managerRefusSI(String idDemande, String managerUsername, String messageRefus) {
		...
		if (demande.getManager() == null && demande.getDemandeur().getManager() == refuseur
			...
			emailNotificationService.notify(demande.getDemandeur().getEmail(), OBJET_SI,
					MESSAGE_REFUS + messageRefus + " REFUSER PAR => " + demande.getRefuseur());
		}
	}
	...
	public void dsiRefusAM(String idDemande, String dsiUsername, String messageRefus) {
		...
		if (demande.getDsi() == null && demande.getDemandeur().getDsi() == refuseur
				&& demande.getEtatDemande() != (byte) 1) {
